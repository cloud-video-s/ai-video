...
function renderResponseParameters(items=[]){if(!items.length)return '<div class="sub">无</div>';return '<table><thead><tr><th>名称</th><th>必有</th><th>类型/约束</th><th>中文说明</th></tr></thead><tbody>'+items.map(p=>'<tr><td><code>'+esc(p.name)+'</code></td><td>'+(p.required?'是':'否')+'</td><td><code>'+esc(schemaSummary(p.schema))+'</code></td><td>'+esc(p.description||p.schema?.description||'')+'</td></tr>').join('')+'</tbody></table>'}
