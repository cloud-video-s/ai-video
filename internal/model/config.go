...
type VideoConfig struct {
	...
	Type      string `json:"type" gorm:"size:16;default:string;comment:string/int/bool/float/json/text/select/password/color"`
	Options   string `json:"options" gorm:"type:text;comment:select 选项或校验规则(JSON)"`
	Sensitive bool   `json:"sensitive" gorm:"default:false;comment:后台响应是否隐藏值"`
