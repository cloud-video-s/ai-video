...
func TestOrderRepoPageAdminAssociatesPurchaser(t *testing.T) {
	...
	if err := db.Exec(`INSERT INTO video_order (
		...
		status, pay_type, third_order_no, original_transaction_id,
		payment_evidence, pay_time, created_at, updated_at
	) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		"order-1", "request-1", 7, 1, 3, "vip-monthly", "VIP Monthly", "USD",
		19.99, 0, 19.99, 19.99, 2.5, 0, 1, 30, domain.OrderStatusPaid, domain.PaymentMethodAppleIAP,
