...
func SeedCountryAdmin() error {
	return DB.Transaction(func(tx *gorm.DB) error {
		...
		apiSeeds := []countryAPISeed{
			{Path: "/admin/countries", Method: "GET", Description: "国家列表"},
			{Path: "/admin/countries/:id", Method: "GET", Description: "国家详情"},
			{Path: "/admin/countries", Method: "POST", Description: "新增国家"},
			{Path: "/admin/countries/:id", Method: "PUT", Description: "编辑国家"},
			{Path: "/admin/countries/:id/status", Method: "PATCH", Description: "切换国家状态"},
			{Path: "/admin/countries/:id", Method: "DELETE", Description: "删除国家"},
		}
		apis := make([]model.VideoAPI, 0, len(apiSeeds))
		for _, seed := range apiSeeds {
			api, err := upsertCountryAPI(tx, seed)
			if err != nil {
				return err
			}
		...
		for _, seed := range buttonSeeds {
			button, err := upsertCountryMenu(tx, seed.menu)
			if err != nil {
				return err
			}
...
func seedDefaultCountries(tx *gorm.DB) error {
	var count int64
	if err := tx.Model(&model.VideoCountry{}).Count(&count).Error; err != nil {
		return err
	}
	if count > 0 {
		return nil
	}
	rows := strings.Split(strings.TrimSpace(defaultCountryData), "\n")
	countries := make([]model.VideoCountry, 0, len(rows))
...
func upsertCountryAPI(tx *gorm.DB, seed countryAPISeed) (*model.VideoAPI, error) {
	...
	default:
		if err := tx.Model(&api).Updates(map[string]interface{}{
			"group": "国家管理", "description": seed.Description,
		}).Error; err != nil {
			return nil, err
		}
	}
...
func upsertCountryMenu(tx *gorm.DB, desired model.VideoMenu) (*model.VideoMenu, error) {
...
const defaultCountryData = `AD|安道尔
AE|阿拉伯联合酋长国
AF|阿富汗
AG|安提瓜和巴布达
AI|安圭拉
