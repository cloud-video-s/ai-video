package handler

import (
	"strconv"

	"ai-video/internal/pkg/errcode"
	"ai-video/internal/pkg/response"
	"ai-video/internal/pkg/utils"
	"ai-video/internal/server/admin/service"

	"github.com/gin-gonic/gin"
)

type PlatformHandler struct{ svc *service.PlatformService }

func NewPlatformHandler() *PlatformHandler {
	return &PlatformHandler{svc: service.NewPlatformService()}
}

func (h *PlatformHandler) List(c *gin.Context) {
	var req service.ListPlatformRequest
	if err := c.ShouldBindQuery(&req); err != nil {
		response.Fail(c, errcode.ErrParam, "参数错误: "+err.Error())
		return
	}
	p := utils.GetPagination(c)
	items, total, err := h.svc.List(c.Request.Context(), p.Page, p.PageSize, &req)
	if err != nil {
		response.Fail(c, errcode.ErrServer, err.Error())
		return
	}
	response.OK(c, gin.H{"list": items, "total": total, "page": p.Page, "size": p.PageSize})
}

func (h *PlatformHandler) ListOptions(c *gin.Context) {
	items, err := h.svc.ListOptions(c.Request.Context())
	if err != nil {
		response.Fail(c, errcode.ErrServer, err.Error())
		return
	}
	response.OK(c, items)
}

func (h *PlatformHandler) Get(c *gin.Context) {
	id, ok := modelManagementID(c, "id", "平台")
	if !ok {
		return
	}
	item, err := h.svc.Get(c.Request.Context(), id)
	if err != nil {
		response.Fail(c, errcode.ErrNotFound, err.Error())
		return
	}
	response.OK(c, item)
}

func (h *PlatformHandler) Create(c *gin.Context) {
	var req service.PlatformPayload
	if err := c.ShouldBindJSON(&req); err != nil {
		response.Fail(c, errcode.ErrParam, "参数错误: "+err.Error())
		return
	}
	item, err := h.svc.Create(c.Request.Context(), &req)
	if err != nil {
		response.Fail(c, errcode.ErrParam, err.Error())
		return
	}
	response.OK(c, item)
}

func (h *PlatformHandler) Update(c *gin.Context) {
	id, ok := modelManagementID(c, "id", "平台")
	if !ok {
		return
	}
	var req service.PlatformPayload
	if err := c.ShouldBindJSON(&req); err != nil {
		response.Fail(c, errcode.ErrParam, "参数错误: "+err.Error())
		return
	}
	item, err := h.svc.Update(c.Request.Context(), id, &req)
	if err != nil {
		response.Fail(c, errcode.ErrParam, err.Error())
		return
	}
	response.OK(c, item)
}

func (h *PlatformHandler) Delete(c *gin.Context) {
	id, ok := modelManagementID(c, "id", "平台")
	if !ok {
		return
	}
	if err := h.svc.Delete(c.Request.Context(), id); err != nil {
		response.Fail(c, errcode.ErrParam, err.Error())
		return
	}
	response.OK(c, nil)
}

func modelManagementID(c *gin.Context, name, resource string) (int64, bool) {
	id, err := strconv.ParseInt(c.Param(name), 10, 64)
	if err != nil || id <= 0 {
		response.Fail(c, errcode.ErrParam, resource+" ID 无效")
		return 0, false
	}
	return id, true
}
