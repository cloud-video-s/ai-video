<template>
  <div class="page-wrap">
    <el-card shadow="never">
      <template #header>
        <div class="page-header">
          <div>
            <div class="page-title">Banner 位置管理</div>
            <div class="page-subtitle">配置 Banner 在客户端的展示入口、封面和启用状态</div>
          </div>
      ...
      <el-table v-loading="loading" :data="tableData" row-key="id" stripe>
        ...
        <el-table-column label="Banner 位置" min-width="190">
          <template #default="{ row }">
            <div class="primary-text">{{ row.placement_name }}</div>
            <el-tag class="placement-key" size="small" type="info" effect="plain">{{ row.placement_key }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column v-if="canEdit || canDelete" label="操作" width="130" fixed="right" align="center">
          <template #default="{ row }">
            ...
            <el-popconfirm
              v-if="canDelete"
              title="确认删除该 Banner 位置？已被 Banner 使用时无法删除。"
              width="280"
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑 Banner 位置' : '新增 Banner 位置'" width="680px" destroy-on-close>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="96px">
        <div class="form-grid">
          <el-form-item label="位置名称" prop="placement_name">
            <el-input v-model="form.placement_name" maxlength="128" placeholder="例如：首页 Banner" />
          </el-form-item>
          <el-form-item label="位置标识" prop="placement_key">
            <el-input v-model="form.placement_key" maxlength="64" placeholder="例如：homeBanner" />
          </el-form-item>
...
const canEdit = computed(() => userStore.hasPermission('banner:placement:edit'))
const canDelete = computed(() => userStore.hasPermission('banner:placement:delete'))
...
const rules: FormRules = {
  placement_name: [{ required: true, message: '请输入位置名称', trigger: 'blur' }],
  placement_key: [
    { required: true, message: '请输入位置标识', trigger: 'blur' },
    { pattern: /^[A-Za-z0-9_-]+$/, message: '仅支持字母、数字、下划线和中划线', trigger: 'blur' },
  ],
  cover_image: [{ required: true, message: '请输入或上传封面图', trigger: 'blur' }],
}
...
function openEdit(row: BannerPlacement) {
  Object.assign(form, {
    id: row.id,
...
async function handleSubmit() {
  ...
  try {
    const payload: BannerPlacementPayload = {
    ...
    if (form.id) await updateBannerPlacement(form.id, payload)
    else await createBannerPlacement(payload)
    ElMessage.success('Banner 位置已保存')
...
async function handleDelete(id: number) {
  await deleteBannerPlacement(id)
  ElMessage.success('Banner 位置已删除')
...
.placement-key { margin-top: 6px; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; }
